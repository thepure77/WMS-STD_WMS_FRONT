...
(function () {
    'use strict'
    app.directive('planGrPop', ['ngAuthSettings', '$window', 'commonService', 'pageLoading', '$timeout',
        function (ngAuthSettings, $window, commonService, pageLoading, $timeout) {
            return {
                restrict: 'E',
                controllerAs: '$ctrl',
                templateUrl: "modules/component/Popup/planGrPopup/GR/planGrPopup.html",
                scope: {
                    onShow: '=',
                    delegates: '=?',
                    invokes: '=?',
                ...
                controller: ['$scope', '$http', '$q', 'ngAuthSettings', '$state', 'pageLoading', '$window', 'commonService', '$timeout', '$translate', 'localStorageService', '$interval', 'planGrPopupFactory',
                    function ($scope, $http, $q, ngAuthSettings, $state, pageLoading, $window, commonService, $timeout, $translate, localStorageService, $interval, planGrPopupFactory) {
                        $scope.delegates = {};
                        $scope.invokes = $scope.invokes || {};
                        $scope.config = $scope.config || {};
                        var viewModel = planGrPopupFactory;
                        ...
                        $scope.find = function (data) {
                            ...
                            $scope.planGr = $scope.planGr || {};
                            $scope.planGr.Chk = $scope.index;
                            $scope.planGr.planGoodsReceive_No = data;
                            $scope.planGr.DocumentType_Index = $scope.documentTypeIndex;
                            $scope.planGr.currentPage = 1;
                            $scope.planGr.perPage = $scope.model.numPerPage;
                            $scope.planGr.owner_Index = $scope.ownerIndex;
