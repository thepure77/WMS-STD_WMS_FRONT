...
(function () {
    'use strict'
app.directive('productPopupCy', ['ngAuthSettings', '$window', 'commonService', 'pageLoading', '$timeout',
function (ngAuthSettings, $window, commonService, pageLoading, $timeout) {
    return {
        restrict: 'E',
        controllerAs: '$ctrl',
        templateUrl: "modules/component/Popup/productPopup/cyclecountPopup/productPopup.html",
        scope: {
            onShow: '=',
            delegates: '=?',
            invokes: '=?',
        ...
        controller: ['$scope', '$http', '$q', 'ngAuthSettings', '$state', 'pageLoading', '$window', 'commonService', '$timeout', '$translate', 'localStorageService', '$interval', 'cyclecountFactory','webServiceAPI',
            function ($scope, $http, $q, ngAuthSettings, $state, pageLoading, $window, commonService, $timeout, $translate, localStorageService, $interval, cyclecountFactory,webServiceAPI) {
                ...
                $scope.product = {};
                        ...
                        $scope.delegates.productPopupCy = function (param) {
                            $scope.product.key = "";
                        ...
                        $scope.search = function (model) {
                            ...
                            viewModel.popupProductfilter(model).then(
                        ...
                        $scope.find = function (data) {
                            ...
                            $scope.product = $scope.product || {};
                            $scope.product.cycleCount_No = data;
                            $scope.product.currentPage = 1;
                            $scope.product.perPage = $scope.model.numPerPage;
                        ...
                        $scope.datalist = {
                            ...
                            invokes: {
                                page: function (param) {
                                    ...
                                    $scope.product.perPage = param.numPerPage;
                                    $scope.searchFilter($scope.product).then(function success(res) {
                                        if ($scope.datalist.delegates.set)
                                        $scope.datalist.delegates.set(res.data);
                                    }, function error(res) { });
