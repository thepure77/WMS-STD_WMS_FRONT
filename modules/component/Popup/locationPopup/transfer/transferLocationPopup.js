...
(function () {
    'use strict'
    app.directive('transferLocationPopup', ['ngAuthSettings', '$window', 'commonService', 'pageLoading', '$timeout',
        function (ngAuthSettings, $window, commonService, pageLoading, $timeout) {
            return {
                restrict: 'E',
                controllerAs: '$ctrl',
                templateUrl: "modules/component/Popup/locationPopup/transfer/transferLocationPopup.html",
                scope: {
                    onShow: '=',
                    delegates: '=?',
                    invokes: '=?',
                ...
                controller: ['$scope', '$http', '$q', 'ngAuthSettings', '$state', 'pageLoading', '$window', 'commonService', '$timeout', '$translate', 'localStorageService', '$interval', 'reasonCodeGiFactory',
                    function ($scope, $http, $q, ngAuthSettings, $state, pageLoading, $window, commonService, $timeout, $translate, localStorageService, $interval, reasonCodeGiFactory) {
                        ...
                        $scope.delegates.transferLocationPopup = function (param, index) {
                            
                            $scope.model = {};
                            $scope.dataset = angular.copy(param);
                            $scope.index = angular.copy(index);
                            if ($scope.datalist.delegates.set)
                                $scope.datalist.delegates.set($scope.dataset,index);
