...
(function () {
    'use strict'
    app.directive('productConversionPopupPlan', ['ngAuthSettings', '$window', 'commonService', 'pageLoading', '$timeout',
        function (ngAuthSettings, $window, commonService, pageLoading, $timeout) {
            return {
                restrict: 'E',
                controllerAs: '$ctrl',
                templateUrl: "modules/component/Popup/productConversion/PlanGR/productConversionPlan.html",
                scope: {
                    onShow: '=',
                    delegates: '=?',
                    invokes: '=?',
                ...
                controller: ['$scope', '$http', '$q', 'ngAuthSettings', '$state', 'pageLoading', '$window', 'commonService', '$timeout', '$translate', 'localStorageService', '$interval', 'productConversionFactoryPlan',
                    function ($scope, $http, $q, ngAuthSettings, $state, pageLoading, $window, commonService, $timeout, $translate, localStorageService, $interval, productConversionFactoryPlan) {
                        $scope.delegates = {};
                        $scope.invokes = $scope.invokes || {};
                        $scope.config = $scope.config || {};
                        var viewModel = productConversionFactoryPlan;
                        ...
                        $scope.searchFilter = function (model) {
                            var deferred = $q.defer();
                            model.product_Index = $scope.index;
                        ...
                        $scope.find = function () {
                            ...
                            $scope.productConversion.currentPage = 0;
                            $scope.productConversion.product_Index = $scope.index;
                            $scope.productConversion.numPerPage = $scope.model.numPerPage;
                            $scope.search($scope.productConversion).then(function success(res) {
                                $scope.datalist.items = res.data;
                                if ($scope.datalist.delegates.set)
