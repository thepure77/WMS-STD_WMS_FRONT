...
(function () {
    'use strict'
    app.directive('manyToOnePopup', ['ngAuthSettings', '$window', 'commonService', 'pageLoading', '$timeout',
        function (ngAuthSettings, $window, commonService, pageLoading, $timeout) {
            return {
                restrict: 'E',
                controllerAs: '$ctrl',
                templateUrl: "modules/component/Popup/LPNmanyToOnePopup/LPN/manyToOnePopup.html",
                scope: {
                    onShow: '=',
                    delegates: '=?',
                    invokes: '=?',
                ...
                controller: ['$scope', '$http', '$q', 'ngAuthSettings', '$state', 'pageLoading', '$window', 'commonService', '$timeout', '$translate', 'localStorageService', '$interval', 'reasonCodeGiFactory',
                    function ($scope, $http, $q, ngAuthSettings, $state, pageLoading, $window, commonService, $timeout, $translate, localStorageService, $interval, reasonCodeGiFactory) {
                        ...
                        $scope.delegates.manyToOnePopup = function (param, index) {
                            ...
                            for (var i = 0; i <= $scope.dataset.items.length - 1; i++) {
                                $scope.dataset.items[i].remainQty = $scope.dataset.items[i].remainingQty;
                            } 

                            $scope.items = $scope.dataset.items.filter(c => c.isCheck);

                            if ($scope.datalist.delegates.set)
                                        $scope.datalist.delegates.set($scope.items);
                        }
