(function () {
    'use strict';
    app.factory("reportPickingFactory",
        function ($q, $http, ngAuthSettings, localStorageService,  webServiceAPI, clientService) {
 
            return {
                get: clientService.get,
                post: clientService.post,
                url: webServiceAPI.Report + "ReportPicking",
                url2: webServiceAPI.Report + "Report21",
                urlDropdown: webServiceAPI.Report + "DropdownReport",
                urlDropdown_2: webServiceAPI.PlanGI + "DropdownPlanGoodsIssue",
                ...
                PrintReport: function (model) {
                    var urlRequest = this.url+ "/printReportPicking";
                ...
                getLocationType: function (model) {
                    var urlRequest = this.url2+ "/getLocationType";
                    return clientService.post(urlRequest, model);
                },
                dropdownItemStatus: function (model) {
                    var urlRequest = this.urlDropdown_2 + "/dropdownItemStatus";
                    return clientService.post(urlRequest, model);
                },
            }
        });
