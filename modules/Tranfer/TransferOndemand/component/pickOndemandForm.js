(function () {
    ...
    app.component('pickOndemandForm', {
        controllerAs: '$vm',
        templateUrl: "modules/Tranfer/TransferOndemand/component/pickOndemandForm.html",
        bindings: {
            isLoading: '=?',
            onShow: '=?',
            searchResultModel: '=?',
        ...
        controller: function ($scope, $q, $filter, $http, /*ngAuthSettings,*/ $state, /*authService*/ pageLoading, $window, commonService, localStorageService, $timeout, $translate, dpMessageBox, planGoodsReceiveFactory, planGoodsReceiveItemFactory, planGoodsIssueItemFactory, ownerPopFactory, webServiceAPI, transferFactory, goodIssueFactory) {
            var $vm = this;
            ...
            $scope.back = function () {
                ...
                if ($scope.formPage == "GT") {
                    ...
                    __viewModel.setParam($scope.itemsReserve)
                    $state.go('wms.transfer_summary', { a: "" });
                }
            }
                ...
                invokes: {
                    ...
                    selected: function (param) {
                        ...
                        if ($scope.formPgae == "GI") {
                            // param.qty = null;
                            if ($scope.dataGiitem) {
                                if ((param.Pick * param.unit.productconversion_Ratio) > $scope.dataGiitem.qty && param.product_Id == $scope.dataGiitem.product_Id) {
                                    return dpMessageBox.alert(
                                        {
                                            ok: 'Close',
                                            title: 'ALERT',
                            ...
                            if ($scope.dataGT) {
                                if ((param.Pick * param.unit.productconversion_Ratio) > (param.qty)) {
                                    return dpMessageBox.alert(
                                        {
                                            ok: 'Close',
                                            title: 'ALERT',
                            ...
                            transferFactory.pickProduct(param).then(function (res) {
                                pageLoading.hide()
                                if (res.data.resultIsUse) {
                                    $scope.itemsReserve.item = $scope.itemsReserve.item || [];
                                    $scope.itemsReserve.item.push(angular.copy(res.data.items));
            ...
            $scope.ok = function () {
                ...
                transferFactory.ListPickProduct(obj).then(function (res) {
                    pageLoading.hide()
            ...
            var init = function () {
                ...
                $scope.dataGT = transferFactory.getParam();
                if ($scope.dataGi) {
