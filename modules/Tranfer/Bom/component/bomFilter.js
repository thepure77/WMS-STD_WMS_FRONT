(function () {
    'use strict';
    app.component('bomFilter', {
        controllerAs: '$vm',
        templateUrl: function ($element, $attrs, $window, commonService) {
            return "modules/Tranfer/BOM/component/bomFilter.html";
        },
        bindings: {
            searchResultModel: '=?',
            filterModel: '=?',
        ...
        controller: function ($scope, $q, $http, $filter, $state, $window, $element, $timeout, $translate, pageLoading, dpMessageBox, localStorageService, commonService, webServiceAPI, bomFactory) {
            var $vm = this;
            ...
            $scope.filterSearch = function () {
                ...
                if ($scope.filterModel.BOM_Index != undefined) {
                    if ($scope.filterModel.BOM_Index == "" || $scope.filterModel.BOM_Index.length < 36) //|| $scope.filterModel.BOM_Index.length < 36
                    {
                        $scope.filterModel.BOM_Index = "00000000-0000-0000-0000-000000000000";
                    }
                }
            ...
            $scope.convertDate = function () {
                ...
                if ($scope.filterModel.date != null) {
                    ...
                    $scope.filterModel.BOM_Date = YStart.toString() + MStart.toString() + DStart.toString();

                    var DEnd = str.substring(15, 13);
                    var MEnd = str.substring(18, 16);
                    var YEnd = str.substring(25, 19);

                    $scope.filterModel.BOM_Date_To = YEnd.toString() + MEnd.toString() + DEnd.toString();
                }
                if ($scope.filterModel.dateDue != null) {
                    ...
                    $scope.filterModel.BOM_Due_Date = YStart.toString() + MStart.toString() + DStart.toString();

                    var DEnd = str.substring(15, 13);
                    var MEnd = str.substring(18, 16);
                    var YEnd = str.substring(25, 19);

                    $scope.filterModel.BOM_Due_Date_To = YEnd.toString() + MEnd.toString() + DEnd.toString();
                }
            ...
            this.$onInit = function () {
                ...
                $scope.filterModel.BOM_Date_To = getToday();
