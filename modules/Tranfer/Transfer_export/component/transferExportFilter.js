(function () {
    'use strict';
    app.component('transferExportFilter', {
        controllerAs: '$vm',
        templateUrl: function ($element, $attrs, /*ngAuthSettings,*/ $window, commonService) {
            return "modules/Tranfer/Transfer_export/component/transferExportFilter.html";
        },
        bindings: {
            searchResultModel: '=?',
            filterModel: '=?',
        ...
        controller: function ($scope, $q, $http, $filter, $state, $window, $element, $timeout, $translate, /*ngAuthSettings,*/ pageLoading, dpMessageBox, localStorageService, commonService, transferExportFactory, webServiceAPI) {
            var $vm = this;
            ...
            $scope.selectSort = [
                {
                    value: "GoodsTransfer_No",
                    display: "เลขที่ใบโอนย้าย"
                },
                {
                    value: "GoodsTransfer_Date",
                    display: "วันที่โอนย้าย"
                },
                {
            ...
            $scope.filterSearch = function () {
                ...
                // $scope.filterModel.goodsTransfer_Date = $vm.filterModel.goodsTransfer_Date;
                // $scope.filterModel.goodsTransfer_Date_To = $vm.filterModel.goodsTransfer_Date_To;
            ...
            $scope.dropdownDocumentType = function () {
                viewModel.dropdownDocumentType($scope.filterModels).then(function (res) {
                    ...
                    $('#documentType_Id').val($scope.dropdownDocumentTypeList[17].documentType_Name);
                });
            };
