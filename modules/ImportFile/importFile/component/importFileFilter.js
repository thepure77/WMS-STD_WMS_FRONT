(function () {
  'use strict';
  app.component('importFileFilter', {
    controllerAs: '$vm',
    templateUrl: function ($element, $attrs, /*ngAuthSettings,*/ $window, commonService) {
      return "modules/ImportFile/importFile/component/importFileFilter.html";
    },
    bindings: {
      searchResultModel: '=?',
      invalidResultModel: '=?',
    ...
    controller: function ($scope, $q, $http, $filter, $state, $window, $element, $timeout, $translate, dpMessageBox, pageLoading, commonService, importFileFactory, webServiceAPI, localStorageService) {
      var $vm = this;
            ...
            if (reader.readAsBinaryString) {
              reader.onload = function (e) {
                ...
                if (route == "PlanGoodsReceive") {
                  viewModel.importsPGR(dataInsert).then(function (res) {
                    pageLoading.hide();
                  ...
                  viewModel.importsPGI(dataInsert).then(function (res) {
                    pageLoading.hide();
                    $vm.guId = { guId: res.data.import_GuID, FileName: $scope.SelectedFile.name };
                    $vm.filterModel.searchResultModel = angular.copy(res.data.valid_Data);
                    $vm.filterModel.invalidResultModel = angular.copy(res.data.invalid_Data);
