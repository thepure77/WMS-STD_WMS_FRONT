(function () {
    ...
    app.component('scanPickQtyForm', {
        controllerAs: '$vm',
        templateUrl: "modules/GI/scanPickQty/component/scanPickQtyForm.html",
        bindings: {
            isLoading: '=?',
            onShow: '=?',
            searchResultModel: '=?',
        ...
        controller: function ($scope, $q, $filter, $http, $location, $anchorScroll, /*ngAuthSettings,*/ $state, /*authService*/ pageLoading, $window, commonService, localStorageService, $timeout, $translate, dpMessageBox, scanPickQtyFactory, scanPickQtyItemFactory, webServiceAPI) {
            ...
            var viewModelItem = scanPickQtyItemFactory;
            $scope.filterModel = {};
            ...
            $scope.confirmPick = function () {
                ...
                if ($scope.isAlreadyPrint == false) {
                    $scope.isBlock = false;
                    return dpMessageBox.alert(
                        {
                            ok: 'Close',
                            title: 'แจ้งเตือน',
                            message: 'กรุณาสั่งปริ้น Tag ก่อน Confirm'
                        }
                    )
                }
            ...
            $scope.printtag = function () {
                $scope.isPrint = true;
                $scope.filterModel.printer = angular.fromJson($window.localStorage['isPrinter']);
                viewModel.ScanQty_print_tag($scope.filterModel).then(function (res) {
                    if (res.data.resultIsUse) {
                        $scope.isPrint = false;
                        $scope.isAlreadyPrint = true;
                    }
                    else {
                        $scope.isBlock = false;
                        $scope.isAlreadyPrint = false;
                        return dpMessageBox.alert(
                            {
                                ok: 'Close',
                                title: 'แจ้งเตือน',
