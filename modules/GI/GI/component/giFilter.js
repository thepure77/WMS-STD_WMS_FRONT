(function () {
    ...
    app.component('giFilter', {
        ...
        controller: function ($scope, $q, $http, $filter, $state, $window, $element, $timeout, $translate, /*ngAuthSettings,*/ pageLoading, dpMessageBox, commonService, goodIssueFactory, localStorageService, webServiceAPI) {
            var $vm = this;
            ...
            $scope.selectSort = [
                {
                    value: "GoodsIssue_No",
                    display: "เลขที่ใบเบิกสินค้า"
                },
                {
                    value: "GoodsIssue_Date",
                    display: "วันที่แจ้งสั่งซื้อสินค้า"
                },
                {
                    value: "Document_Date",
                    display: "วันที่กาหนดส่งสินค้า"
                },
                {
            ...
            $vm.triggerSearch = function () {
                ...
                $vm.filterModel.goodsIssue_Date = $vm.filterModel.goodsIssue_Date || getToday();
                $vm.filterModel.goodsIssue_Date_To = $vm.filterModel.goodsIssue_Date_To|| getToday();
                $scope.filterModel.advanceSearch = $vm.filterModel.advanceSearch;
                pageLoading.show();
                viewModel.filter($vm.filterModel).then(function (res) {
                    pageLoading.hide();
                    if (res.data.itemsGI.length != 0) {
                        ...
                        $vm.searchResultModel = res.data.itemsGI;
                    }
                    else {
                        $vm.searchResultModel = res.data.itemsGI;
                    }
