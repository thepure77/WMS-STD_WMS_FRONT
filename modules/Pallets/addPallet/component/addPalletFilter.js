(function () {
    'use strict';
    app.component('addPalletFilter', {
        controllerAs: '$vm',
        templateUrl: function ($element, $attrs, $window, commonService) {
            return "modules/Pallets/addPallet/component/addPalletFilter.html";
        },
        bindings: {
            searchResultModel: '=?',
            filterModel: '=?',
        ...
        controller: function ($scope, $q, $http, $filter, $state, $window, $element, $timeout, $translate, pageLoading, dpMessageBox, localStorageService, commonService, webServiceAPI,importFileFactory,addPalletFactory) {
            var $vm = this;

            // ----------------------------------------------------
            var model = $scope.filterModel;
            var viewModel = addPalletFactory;
            $scope.filterSumModel = {};
            $vm.filterModel = {
                currentPage: 1,
                perPage: 50,
                totalRow: 0,
            ...
            $scope.filterSum = function () {
                //pageLoading.show()
                viewModel.FilterSum().then(
                    function success(res) {
                        $scope.filterSumModel = res.data;
                        //pageLoading.hide()
                        //deferred.resolve(res);
                    },
                    function error(response) {
            ...
            function ExportExcel() {
                ...
                var deferred = $q.defer();
                $scope.filterModel.excelName = "Report Add Pallets";
