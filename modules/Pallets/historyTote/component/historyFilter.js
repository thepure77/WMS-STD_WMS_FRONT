(function () {
    'use strict';
    app.component('plantFilter', {
        controllerAs: '$vm',
        templateUrl: function ($element, $attrs, /*ngAuthSettings,*/ $window, commonService) {
            return "modules/masterData/masterPlant/component/plantFilter.html";
        },
        bindings: {
            searchResultModel: '=?',
            filterModel: '=?',
        ...
        controller: function ($scope, $q, $http, $filter, $state, $window, $element, $timeout, $translate, /*ngAuthSettings,*/ pageLoading, commonService,plantFactory,/*soldToFactory,*/webServiceAPI) {
            var $vm = this;
            ...
            $scope.export = function () {
                ...
                viewModel.Export($scope.exportModel).then(function (res) {
                    ...
                    debugger
                    if (res.data.length != 0) {
                        $vm.ResultModelExcel = res.data.itemsPlant;

                        var createXLSLFormatObj = [];

                        /* XLS Head Columns */
                        var xlsHeader = ["ลำดับ","รหัส Plant", "ชื่อ Plant", "ผู้สร้าง","วันที่สร้างเอกสาร","ผู้แก้ไข","วันที่แก้ไข","ผู้ยกเลิก","วันที่ยกเลิก","สถานะ"];
                        ...
                        $vm.ResultModelExcel.forEach(e => {
                            xlsRows.push({
                                "ลำดับ": e.numBerOf,
                                "รหัส Plant": e.plant_Id,
                                "ชื่อ Plant": e.plant_Name,
                        ...
                        pageLoading.hide();
                        JSONToCSVConvertor(createXLSLFormatObj, "คลัง");
                    } else {
                        $vm.ResultModelExcel = res.data.itemsPlant;
                    }
                });
            ...
            $scope.autoComplete = {
                plantName: "autoPlant/autoSearchPlantFilter"
