(function () {
    ...
    app.component('sccsMappingForm', {
        controllerAs: '$vm',
        templateUrl: function ($element, $attrs, /*ngAuthSettings,*/ $window, commonService) {
            return "modules/SccsMapping/sccsMapping/component/sccsMappingForm.html";
        },
        bindings: {
            onShow: '=?',
            searchResultModel: '=?',
        ...
        controller: function ($filter, dpMessageBox, $scope, $q, $window, productFactory, locationFactory, localStorageService, productCategoryFactory, pageLoading, webServiceAPI, zoneFactory, replenishmentFactory) {
            var $vm = this;
            $scope.onShow = false;
            var defer = {};
            $scope.Cancel = true;
            ...
            $vm.onShow = function (param) {
                ...
                if (param != undefined) {
                    ...
                    viewModelReplenishment.find(param).then(function (res) {
                        pageLoading.hide();
                        var models = res.data;
                        $scope.dtChoose1 = models.replenishmentProducts;
                        $scope.dtChooseLocation = models.replenishmentLocations;
                        if (models.isMonday == true || models.isTuesday == true || models.isWednesday == true || models.isThursday == true ||
                            models.isFriday == true || models.isSaturday == true || models.isSunday == true || models.chkDateAll == true) {
                            models.trigger_Date = null;
                            models.trigger_Date_End = null;
                        }

                        let checkVar = 0;
                        if (models.trigger_Date != null) {
                            var ds = formatDate(models.trigger_Date);
                            models.trigger_Date = ds;
                            checkVar = 1;
                        }
                        if (models.trigger_Date_End != null) {
                            var de = formatDate(models.trigger_Date_End);
                            models.trigger_Date_End = de;
                            checkVar = 1
                        } else {
                            checkVar = 0
                        }
                        ...
                        if (models.plan_By_Product == 2) {
                            models.chkAllProduct = true;
                        }
                        if (models.plan_By_Location == 2) {
                            models.chkallLocation = true;
                        }
                        if (models.isActive == 1) {
                            models.isActive = true;
                        }

                        if (
                            models.isMonday == true &&
                            models.isTuesday == true &&
                            models.isWednesday == true &&
                            models.isThursday == true &&
                            models.isFriday == true &&
                            models.isSaturday == true &&
                            models.isSunday == true) {

                            models.chkDateAll = true;

                        } else {
                            models.chkDateAll = false;

                        }

                        $scope.filterModel = models;
                        $scope.create = false;
                        $scope.update = true;
                    });
                }
            ...
            $scope.saveReplenishment = function () {
                ...
                if (validateTimePopup(model) == false) {
                    return;
                }

                if (validateDatePopup(model) == false) {
                    return;
                }

                if (checkItemProductAndLocation(model) == false) {
                    return;
                }
                if (model.trigger_Date != null) {
                    var ds = convertDate(model.trigger_Date);
                    model.trigger_Date = ds;
                }
                if (model.trigger_Date_End != null) {
                    var de = convertDate(model.trigger_Date_End);
                    model.trigger_Date_End = de;
                }

                if ($scope.dtChoose.length > 1) {
                    var datalistProduct = sentProductModels($scope.dtChoose);
                    model.replenishmentProducts = datalistProduct;
                ...
                if ($scope.dtChoose1 != undefined) {
                    if ($scope.dtChoose1.length > 1) {
                        var datalistProduct = sentProductModels($scope.dtChoose1);
                        model.replenishmentProducts = datalistProduct;
                ...
                if ($scope.dtChooseLocation.length > 1) {
                    var datalistLocation = sentProductModels($scope.dtChooseLocation);
                    model.replenishmentLocations = datalistLocation;
                ...
                if (checkLanguage.name == 'TH') {
                    messagebox.product = 'กรุณาเลือกสินค้ามากกว่า 1 รายการ'
                    messagebox.location = 'กรุณาเลือกตำแหน่งมากกว่า 1 รายการ'
                }
                else {
                    messagebox.product = 'Please Fill article more 1 choice !'
                    messagebox.location = 'Please Fill Location more 1 choice !'

                }
                // Validate
                if (model.replenishmentProducts.length <= 1) {
                    dpMessageBox.alert({
                        ok: 'Close',
                        title: 'Validate',
                        message: messagebox.product
                    })

                    $scope.models.appointment_Date = getToday();
                    return "";
                }
                if (model.replenishmentLocations.length <= 1) {
                    dpMessageBox.alert({
                        ok: 'Close',
                        title: 'Validate',
                        message: messagebox.location
                    })
