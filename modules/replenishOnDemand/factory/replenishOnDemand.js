(function () {
    'use strict';
    app.factory("replenishOndemandFactory",
        function ($q, $http, ngAuthSettings, localStorageService,  webServiceAPI, clientService) {
 
            return {
                get: clientService.get,
                post: clientService.post,
                url: webServiceAPI.Replenishment + "replenishOnDemand",
                urlWcs: webServiceAPI.WCS + "WMS",
                urlDropdown: webServiceAPI.Cyclecount + "DropdownCyclecount",
                urlLogsRequest: webServiceAPI.LogsUser + "Logs/insertLogsRequest",

                dropdownRoundWave: function (model) {
                    var urlRequest = this.url + "/dropdownRoundWave";
                    return clientService.post(urlRequest, model);
                },
                filter: function (model) {
                    var urlRequest = this.url + "/filterReplenishOnDemand";
                    return clientService.post(urlRequest, model);
                },
                confirmTransferAndSendWCSBypass: function (model) {
                    var urlRequest = this.url + "/confirmTransferAndSendWCSBypass";
                    return clientService.post(urlRequest, model);
                },
                WCSBypass: function (model) {
                    var urlRequest = this.urlWcs + "/PREPARE_TRANSFER";
                    return clientService.post(urlRequest, model);
                },
                create_bypassno: function (model) {
                    var urlRequest = this.url + "/activateBypassReplenishmentFromASRS";
                    return clientService.post(urlRequest, model);
                },
                create_replenish: function (model) {
                    var urlRequest = this.url + "/activePIECEPICKOnDemand";
                    return clientService.post(urlRequest, model);
                },
                getTranferOnDemand: function () {                    
                    var urlRequest = this.url + "/getTranferOnDemand";
