    ...
    app.factory("planGoodsReceiveFactory",
        function ($q, $http, ngAuthSettings, localStorageService,  webServiceAPI, clientService) {
            ...
            return {
                ...
                urlDropdown: webServiceAPI.PlanGR + "DropdownPlanGoodsReceive",
                urlLogsRequest: webServiceAPI.LogsUser + "Logs/insertLogsRequest",
                urlOMS: webServiceAPI.OMS + "OMStoAMZ",
                urlClaim: webServiceAPI.OMS + "Claim",
                urlBilling: webServiceAPI.OMS + "Billing",

                TransferGR: function (model) {
                    var urlRequest = this.urlOMS +"/TranferGR";
                ...
                filter_Exprot: function (model){
                    var urlRequest = this.url+ "/filter_Exprot";
                    return clientService.post(urlRequest, model);
                }, 
                ...
                PrintPlanGoodsReceive: function (model) {
                    var urlRequest = this.url+ "/PrintPlanGoodsReceive";
                ...
                getBilling: function (model) {
                    var urlRequest = this.urlBilling + "/Send_Billing/" + model;
                    return clientService.get(urlRequest);
                },
                getscanClaim: function (model) {
                    var urlRequest = this.urlClaim + "/Send_Claim/" + model;
                    return clientService.get(urlRequest);
                },
        ...
        function logsRequest(operations,urlRequest,model)
        {
            ...
            //logs.sub_Menu_Id
            logs.sub_Menu_Name = "ใบสั่งซื้อสินค้า";
