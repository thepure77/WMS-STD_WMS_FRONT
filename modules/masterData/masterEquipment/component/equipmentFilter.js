...
(function () {
    ...
    app.component('masterEquipmentFilter', {
        ...
        controller: function ($scope, $q, $http, $filter, $state, $window, $element, $timeout, $translate, /*ngAuthSettings,*/ pageLoading, commonService,equipmentFactory,webServiceAPI , dpMessageBox) {
            var $vm = this;
                ...
                viewModel.filter($scope.filterModel).then(function (res) {
                    ...
                    if (res.data.length != 0) {
                        ...
                        if (res.data.pagination != null || res.data.pagination != undefined) {
                            $vm.filterModel.totalRow = res.data.pagination.totalRow;
                            $vm.filterModel.key = $scope.filterModel.key;
                            $vm.filterModel.createdateeq_date = $scope.filterModel.createdateeq_date
                            $vm.filterModel.createdateeq_date_to = $scope.filterModel.createdateeq_date_to
                        }
            ...
            $scope.export = function () {
                ...
                viewModel.Export($scope.exportModel).then(function (res) {
                    //pageLoading.hide();
                    if (res.data.length != 0) {
                        $vm.ResultModelExcel = res.data.itemsEquipment;

                        var createXLSLFormatObj = [];
                        var xlsHeader = ["No.","Equipment Id", "Equipment Name", "Equipment Type" , "Equipment Subtype", "Create By" ,"Create Date","Update By","Update Date"];
                        ...
                        $vm.ResultModelExcel.forEach(e => {
                            xlsRows.push({
                                ...
                                "Equipment Type": e.equipmentType_Name,
                                "Equipment Subtype": e.equipmentSubType_Name,
                        ...
                        pageLoading.hide();
                        JSONToCSVConvertor(createXLSLFormatObj, "Equipment");
                    } else {
                        $vm.ResultModelExcel = res.data.itemsEquipment;
                    }
                });
            ...
            if ($scope.filterModel.createdateeq != null) {
                var str = $scope.filterModel.createdateeq;
    
                var DStart = str.substring(0, 2);
                var MStart = str.substring(5, 3);
                var YStart = str.substring(10, 6);
            
                $scope.filterModel.createdateeq_date = YStart.toString() + MStart.toString() + DStart.toString();
    
                var DEnd = str.substring(15, 13);
                var MEnd = str.substring(18, 16);
                var YEnd = str.substring(25, 19);
    
                $scope.filterModel.createdateeq_date_to = YEnd.toString() + MEnd.toString() + DEnd.toString();
            }
            };
            ...
            $scope.changeSet = function () {
                debugger
                 if ($scope.filterModel.useDate === 1){
                    // $scope.notuseDate();
                    $scope.filterModel.createdateeq = " ";
                 }
                 if ($scope.filterModel.useDate === 0){
                    // $scope.useDate();
                    $scope.filterModel.createdateeq = formatDate();
                 }
