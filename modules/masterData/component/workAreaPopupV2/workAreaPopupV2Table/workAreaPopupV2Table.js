(function() {
    'use strict'
    app.directive('workAreaPopupV2Table', function() {
        return {
            restrict: 'E',
            controllerAs: '$ctrl',
            templateUrl: "modules/masterData/component/workAreaPopupV2/workAreaPopupV2Table/workAreaPopupV2Table.html",
            scope: {
                delegates: '=',
                invokes: '=',
                config: '=',
            ...
            controller: ['$scope', 'commonService', '$filter','dpMessageBox','workAreaPopupV2Factory','$q', function($scope, commonService, $filter,dpMessageBox,workAreaPopupV2Factory,$q) {
                $scope.invokes = $scope.invokes || {};
                $scope.config = $scope.config || {};
                $scope.config.paginations = $scope.config.paginations || {};
                $scope.items = [];
                $scope.items = $scope.items || [];
                $scope.listLocationWorkArea = [];
                ...
                $scope.delegate = {
                    set: function(model, LocationIndex) {
                        $scope.location_Index = LocationIndex;
                        $scope.items = model;    
                    },
                ...
                function Add(select) {
                    $scope.ItemObj = {};
                    $scope.ItemObj.location_Index = $scope.location_Index;
                    $scope.ItemObj.listLocationWorkAreaItemViewModel = select;
