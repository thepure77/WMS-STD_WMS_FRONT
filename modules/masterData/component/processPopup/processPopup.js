...
(function () {
    'use strict'
    app.directive('processPopup', ['ngAuthSettings', '$window', 'commonService', 'pageLoading', '$timeout',
        function (ngAuthSettings, $window, commonService, pageLoading, $timeout) {
            return {
                restrict: 'E',
                controllerAs: '$ctrl',
                templateUrl: "modules/masterData/component/processPopup/processPopup.html",
                scope: {
                    onShow: '=',
                    delegates: '=?',
                    invokes: '=?',
                ...
                controller: ['$scope', '$http', '$q', 'ngAuthSettings', '$state', 'pageLoading', '$window', 'commonService', '$timeout', '$translate', 'localStorageService', '$interval', 'processFactory',
                    function ($scope, $http, $q, ngAuthSettings, $state, pageLoading, $window, commonService, $timeout, $translate, localStorageService, $interval, processFactory) {
                        $scope.delegates = {};
                        $scope.invokes = $scope.invokes || {};
                        $scope.config = $scope.config || {};
                        var viewModel = processFactory;
                        ...
                        $scope.searchFilter = function (model) {
                            ...
                            if (model.processName != null && model.processId == undefined) {
                                for (var i = 0; i <= item.length - 1; i++) {
                                    if (item[i].processName == model.processName) {
                                        model.processId = item[i].processId;
                                    }
                                };
                            }
                        ...
                        $scope.delegates.processPopup = function (param, index) {
